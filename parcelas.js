...
router.get('/', (req, res) => {
  ...
  res.json(db.parcelas.all(filtros));
});

router.get('/vencendo', (req, res) => {
  const dias = parseInt(req.query.dias) || 7;
  res.json(db.parcelas.vencendo(dias));
});

router.put('/:id/pagar', (req, res) => {
  const parcela = db.parcelas.pagar(req.params.id, req.body.data_pagamento);
  if (!parcela) return res.status(404).json({ erro: 'Parcela nao encontrada' });
  res.json(parcela);
});

router.put('/:id/cancelar', (req, res) => {
  const parcela = db.parcelas.cancelar(req.params.id);
  if (!parcela) return res.status(404).json({ erro: 'Parcela nao encontrada' });
  res.json(parcela);
});
