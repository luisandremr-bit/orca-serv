...
const FILES = {
  ...
  itens: path.join(DATA_DIR, 'itens.json'),
  parcelas: path.join(DATA_DIR, 'parcelas.json'),
...
const db = {
  ...
  configuracoes: {
    get() {
      ...
      const raw = fs.readFileSync(FILES.configuracoes, 'utf8');
      return JSON.parse(raw);
    },
    save(dados) {
      save(FILES.configuracoes, dados);
      return dados;
    }
  },
